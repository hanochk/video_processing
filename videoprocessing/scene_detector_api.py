...
def paperspace_playground_mk():
    # base_folder = '/home/hanoch/notebooks/nebula3_videoprocessing/videoprocessing'#'/mnt/share'
    base_folder = '/media/hollywood2/Hollywood2/AVIClips'
    out_folder = '/home/hanoch/mdf_hollywood2/all'#'/home/hanoch/mdf_lsmdc/all'
    ...
    multi_folder = 1
    out_folder_base = out_folder
    for folder in folders: #['3001_21_JUMP_STREET', folders[-1]]: #folders:
        multi_folder = 1
        out_folder = os.path.join(out_folder_base, folder)
        if not os.path.isdir(os.path.join(base_folder, folder)):
            multi_folder = 0
            if os.path.join(base_folder, folder).endswith('avi') or os.path.join(base_folder, folder).endswith('mp4'):
                videos = os.path.join(base_folder, folder)
                out_folder = os.path.join(out_folder_base, folder.split('.')[0])
            else:
                print("folder does not exist", os.path.isdir(os.path.join(base_folder, folder)))
                continue
        if not os.path.exists(out_folder):
            os.makedirs(out_folder)
        if multi_folder:
            videos = glob.glob(os.path.join(base_folder, folder) + '/**/*', recursive=True)
        else:
            videos = [videos]    
            
        for video in videos:
            # if video != '/mnt/share/0001_American_Beauty/0001_American_Beauty_00.14.59.016-00.15.04.887.avi': #'/mnt/share/0001_American_Beauty/0001_American_Beauty_00.05.13.069-00.05.18.464.avi':
            #     continue
            ...
            if multi_folder == 0 and 0:  # @@HK was OK need to further check
                 new_video = video
            else:
