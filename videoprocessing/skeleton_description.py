...
class HumanSkeleton:
    ...
    def process_file(self, path, to_draw=False):
        image = cv.imread(path)
        if image is None:
            return None
        image_height, image_width, _ = image.shape
        # Convert the BGR image to RGB before processing.
        results = self.holistic.process(cv.cvtColor(image, cv.COLOR_BGR2RGB))
...
def test_skeleton_comparison():
    data_folder = '/home/paperspace/data/mdf_test'
    output_folder = '/home/paperspace/data/mdf_test'
    skeleton = HumanSkeleton()
    for f in os.listdir(data_folder):
        result = skeleton.process_file(os.path.join(data_folder, f), to_draw=True)
        if result is None:
            continue
        # cv.imwrite(output_folder + '/' + f[:-4] + '_pose' + '.png', result)
...
def test_yolo():
    ...
    if args.wandb_project is not None:
        from yolov7old.utils.wandb.wandb_logger import WandbInferenceLogger
    ...
    while iter.ok:
        ...
        if args.output:
            ...
            out_path = "frame"
        cv.imwrite(str(out_path), res)
        # len(res['instances'].get_fields()['pred_classes'])
        # res['instances'].get_fields()['scores']
